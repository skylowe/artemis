...
### Employment / LFPR
20. **LFPR trend slope adjustments:** TR2025 uses ~50 unpublished "addfactors" (Input #20) to modify LFPRs "for reasonable age-LFPR profile shape." Without adjustments, negative time trend coefficients for ages 16-29 drive LFPR toward 0% over the 75-year projection. ARTEMIS dampens per-group trend slopes to match BLS 2024-2034 projected rates, then freezes accumulation after a configurable number of years. Only slope corrections are applied (no level shifts) since equations are calibrated to CPS base-year LFPR. Configurable per age-sex group via `economics.employment.lfpr_trend_adjustments` (`rate` and `max_years`).
